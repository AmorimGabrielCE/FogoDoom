...
function updateFireIntensityPerPixel(currentPixelIndex) {
    ...
    const belowPixelFireIntensity = firePixelsArrray[belowPixelIndex]
    const newFireIntensity = 
    belowPixelFireIntensity - decay >= 0 ? belowPixelFireIntensity - decay : 0

    firePixelsArrray[currentPixelIndex - decay] = newFireIntensity
}
...
function renderFire(){
    ...
    for (let row = 0; row < fireHeigth; row++) {
        ...
        for (let column = 0; column < fireWidth; column++) {
            const pixelIndex = column + (fireWidth * row)
            const fireIntensity = firePixelsArrray[pixelIndex]
...
function createFireSource(){
    for (let column = 0; column <= fireWidth; column++) {
        ...
        firePixelsArrray[pixelIndex] = 36
